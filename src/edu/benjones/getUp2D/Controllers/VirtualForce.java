...
public class VirtualForce {
	 ...
	 * 
	 * @param base
	 *            joint to a "grounded" link where the force starts
	 * @param target
	 *            body where the force is desired
	 * @param plocal
	 *            local coordinate on the body where to apply it
	 * @param fGlobal
	 *            desired force in global coordinates
	 */
	public VirtualForce(RevoluteJoint base, Body target, Vec2 pLocal,
			Vec2 fGlobal) {
		this.base = base;
		this.target = target;
		this.pLocal = pLocal;
	 ...
	 *            desired force in global coordinates
	 * @param reversed
	 *            true if base is a "child" of target
	 */
	public VirtualForce(RevoluteJoint base, Body target, Vec2 pLocal,
			Vec2 fGlobal, boolean reversed) {
		this.base = base;
		this.target = target;
		this.pLocal = pLocal;
		this.fGlobal = fGlobal;
	...
	public void apply(HashMap<RevoluteJoint, Integer> contactMap,
			float[] torques) {
		RevoluteJoint curr;
		if (reversed) {
			...
			while (curr != null) {
				...
				torques[contactMap.get(curr)] += torque;
				curr = PhysicsUtils.getChildJoint(curr);
			}
		}
	}

	static final Vec2[] triangle = { new Vec2(0f, -.03f), new Vec2(0f, 0.03f),
			new Vec2(.08f, 0f) };
