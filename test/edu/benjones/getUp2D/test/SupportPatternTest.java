...
public class SupportPatternTest {
	...
	public void setupSP() {
		...
		sp.addLimbStatus(SupportPattern.supportLabel.leftArm, .7f,
				new SupportPattern.supportInfo(
						SupportPattern.limbStatus.stance, false, 0f));
		...
		sp.setTimeWarp(new TimeWarp(0, 10));
	}

	@Test
	public void statusTests() {

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 0).ls,
				limbStatus.idle);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, .4f).ls,
				limbStatus.idle);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, .5f).ls,
				limbStatus.swing);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, .5f).ls,
				limbStatus.swing);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 0.6f).ls,
				limbStatus.swing);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 0.65f).ls,
				limbStatus.swing);
		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 0.7f).ls,
				limbStatus.stance);
		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 0.75f).ls,
				limbStatus.stance);
		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, .95f).ls,
				limbStatus.swing);

		assertEquals(sp.getInfoAtTime(supportLabel.leftArm, 2.0f).ls,
				limbStatus.stance);

		// swingPhaseTests
		assertEquals(MathUtils.floatEquals(
				sp.getSwingPhase(supportLabel.leftArm), 0f), true);
		assertEquals(MathUtils.floatEquals(
				sp.getTimeToLift(supportLabel.leftArm), 0f), true);
		sp.advancePhase(.5f);
		...
		sp.advancePhase(.1f);
		assertEquals(MathUtils.floatEquals(
				sp.getSwingPhase(supportLabel.leftArm), .5f), true);
		sp.advancePhase(.15f);// at .75
		assertEquals(MathUtils.floatEquals(
				sp.getTimeToLift(supportLabel.leftArm), 0.05f), true);
		assertEquals(MathUtils.floatEquals(
				sp.getSwingPhase(supportLabel.leftArm), 0f), true);
		...
		assertEquals(MathUtils.floatEquals(
				sp.getSwingPhase(supportLabel.leftArm), 0.5f), true);
		sp.advancePhase(.3f);// at 1.2
		assertEquals(
				sp.getTimeToLift(supportLabel.leftArm) == Float.POSITIVE_INFINITY,
				true);
